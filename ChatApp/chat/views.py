...
def room(request, room_name):
    room_name_in_db = ChatRoom.objects.filter(name=room_name)
    if not room_name_in_db:
        created_room = ChatRoom(name=room_name)
        created_room.save()
        room_id = created_room.pk
    else:
        room_id = room_name_in_db.pk
    return render(request, 'chat/room.html', {'room_name': room_name, 'room_id': room_id} )